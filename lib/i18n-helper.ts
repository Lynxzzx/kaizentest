import { useRouter } from 'next/router'
import { useState, useEffect, useCallback, useRef } from 'react'
import axios from 'axios'

// Helper functions for i18n translations
// For now, we'll use simple translations until next-i18next is properly configured

// Cache de traduções no cliente
const clientTranslationCache = new Map<string, string>()

// Função para traduzir texto usando API
async function translateText(text: string, from: string = 'pt', to: string = 'en'): Promise<string> {
  // Verificar cache primeiro
  const cacheKey = `${from}-${to}-${text}`
  if (clientTranslationCache.has(cacheKey)) {
    return clientTranslationCache.get(cacheKey)!
  }

  // Se o texto já está em inglês ou é muito curto, retornar como está
  if (to === 'en' && text.length < 3) {
    return text
  }

  try {
    const response = await axios.post('/api/translate', {
      text,
      from,
      to
    })

    const translatedText = response.data.translatedText || text
    
    // Salvar no cache
    clientTranslationCache.set(cacheKey, translatedText)
    
    // Limitar cache a 500 entradas
    if (clientTranslationCache.size > 500) {
      const firstKey = clientTranslationCache.keys().next().value
      if (firstKey) {
        clientTranslationCache.delete(firstKey)
      }
    }

    return translatedText
  } catch (error) {
    console.error('Error translating text:', error)
    return text // Retornar texto original em caso de erro
  }
}

export const translations: Record<string, Record<string, string>> = {
  'pt-BR': {
    'siteName': 'Kaizen Gens',
    'login': 'Entrar',
    'logout': 'Sair',
    'register': 'Registrar',
    'createAccount': 'Criar Conta',
    'username': 'Usuário',
    'password': 'Senha',
    'email': 'Email',
    'admin': 'Admin',
    'dashboard': 'Painel',
    'services': 'Serviços',
    'stocks': 'Estoques',
    'plans': 'Planos',
    'keys': 'Chaves',
    'users': 'Usuários',
    'generate': 'Gerar',
    'account': 'Conta',
    'accounts': 'Contas',
    'create': 'Criar',
    'edit': 'Editar',
    'delete': 'Excluir',
    'save': 'Salvar',
    'cancel': 'Cancelar',
    'name': 'Nome',
    'description': 'Descrição',
    'price': 'Preço',
    'duration': 'Duração (dias)',
    'active': 'Ativo',
    'inactive': 'Inativo',
    'status': 'Status',
    'payment': 'Pagamento',
    'pay': 'Pagar',
    'pix': 'PIX',
    'bitcoin': 'Bitcoin',
    'pending': 'Pendente',
    'paid': 'Pago',
    'expired': 'Expirado',
    'myPlan': 'Meu Plano',
    'expiresAt': 'Expira em',
    'generationsLeft': 'Gerações restantes',
    'unlimited': 'Ilimitado',
    'selectService': 'Selecione um serviço',
    'generateAccount': 'Gerar Conta',
    'available': 'Disponível',
    'unavailable': 'Indisponível',
    'add': 'Adicionar',
    'search': 'Buscar',
    'noResults': 'Nenhum resultado encontrado',
    'confirm': 'Confirmar',
    'areYouSure': 'Tem certeza?',
    'createdAt': 'Criado em',
    'updatedAt': 'Atualizado em',
    'plansDescription': 'Escolha o plano ideal para você e tenha acesso a todos os nossos serviços',
    'mostPopular': 'MAIS POPULAR',
    'daysAccess': 'dias de acesso',
    'generations': 'gerações',
    'accessAllServices': 'Acesso a todos os serviços',
    'payViaPix': 'Pagar via PIX',
    'payViaCrypto': 'Pagar via Criptomoedas',
    'payViaCard': 'Pagar via Cartão',
    'comingSoon': 'Em breve',
    'cardPaymentDescription': 'Preencha os dados do seu cartão de crédito para efetuar o pagamento.',
    'cardNumber': 'Número do Cartão',
    'expMonth': 'Mês',
    'expYear': 'Ano',
    'cardHolderName': 'Nome no Cartão',
    'cardHolderNamePlaceholder': 'NOME COMO ESTÁ NO CARTÃO',
    'cardSecurityNotice': 'Seus dados estão protegidos com criptografia SSL.',
    'cardSecurityNoticeTokenized': 'Seus dados do cartão são processados diretamente pelo PagBank. Nenhum dado sensível é armazenado em nossos servidores.',
    'securePayment': 'Pagamento Seguro',
    'pciCompliant': 'PCI Compliant',
    'sslEncrypted': 'Criptografado SSL',
    'pagbankSecure': 'Processado pelo PagBank',
    'tokenizingCard': 'Tokenizando cartão...',
    'cardTokenized': 'Cartão tokenizado com sucesso',
    'tokenizationError': 'Erro ao tokenizar cartão. Tente novamente.',
    'payNow': 'Pagar Agora',
    'processing': 'Processando...',
    'cardPaymentApproved': 'Pagamento aprovado! Seu plano foi ativado.',
    'cardPaymentPending': 'Pagamento em análise. Aguarde a confirmação.',
    'cardPaymentError': 'Erro ao processar o pagamento. Tente novamente.',
    'invalidCardNumber': 'Número do cartão inválido.',
    'invalidExpMonth': 'Mês de validade inválido.',
    'invalidExpYear': 'Ano de validade inválido.',
    'invalidCvv': 'CVV inválido.',
    'invalidCardHolderName': 'Nome no cartão inválido.',
    'paymentMethodCard': 'Cartão de Crédito',
    'noPlansAvailable': 'Nenhum plano disponível no momento.',
    'informEmail': 'Informe seu email',
    'emailRequired': 'O PagSeguro exige um email válido para processar o pagamento PIX. Este email será usado apenas para a transação.',
    'emailLabel': 'Email *',
    'emailPlaceholder': 'seu@email.com',
    'continue': 'Continuar',
    'close': 'Fechar',
    'paymentVia': 'Pagamento via',
    'creatingPixPayment': 'Criando pagamento PIX...',
    'checkingPayment': 'Verificando pagamento automaticamente... (a cada 5 segundos)',
    'pixCodeCopyPaste': 'Código PIX (Copiar e Colar)',
    'instructions': 'Instruções:',
    'instructionsText': 'Escaneie o QR Code ou copie o código PIX.',
    'autoActivation': '✨ Ativação Automática:',
    'autoActivationText': 'O sistema verifica o pagamento a cada 5 segundos. Assim que o pagamento for confirmado, seu plano será ativado automaticamente!',
    'waitProcessing': 'Aguarde:',
    'waitProcessingText': 'Estamos processando os dados do pagamento PIX. Por favor, recarregue a página em alguns instantes ou verifique o código do pagamento na sua conta.',
    'amountToPay': 'Valor a pagar:',
    'bitcoinAddress': 'Endereço Bitcoin',
    'copy': 'Copiar',
    'network': 'Rede:',
    'sendExactly': 'Envie exatamente',
    'toAddressAbove': 'para o endereço acima.',
    'paymentProcessed': 'O pagamento será processado automaticamente após confirmação na rede.',
    'important': '⚠️ Importante:',
    'importantText': 'Verifique o endereço antes de enviar. Transações de criptomoedas são irreversíveis.',
    'attention': 'Atenção:',
    'autoPaymentUnavailable': 'Sistema de pagamento automático temporariamente indisponível.',
    'contactTelegram': 'Entre em contato via Telegram para completar o pagamento:',
    'openTelegram': 'Abrir Telegram',
    'joinTelegram': 'Entrar no Telegram',
    'joinDiscord': 'Entrar no Discord',
    'freeKeys': 'Lá soltamos Keys de acesso ao gerador totalmente free e muito mais',
    'or': 'ou',
    'loadingQrCode': 'Carregando QR code...',
    'pixPaymentCreated': 'Pagamento PIX criado com sucesso!',
    'paymentConfirmed': '🎉 Pagamento confirmado! Seu plano foi ativado automaticamente!',
    'loginToContinue': 'Faça login para continuar',
    'errorLoadingPlans': 'Erro ao carregar planos',
    'errorCreatingPix': 'Erro ao criar pagamento PIX',
    'errorCreatingPayment': 'Erro ao criar pagamento. Tente novamente.',
    'errorInvalidEmail': 'Por favor, insira um email válido',
    'errorIncompletePaymentData': 'Erro: Dados de pagamento incompletos. Tente novamente.',
    'addressCopied': 'Endereço copiado!',
    'errorCreatingPaymentCrypto': 'Erro ao criar pagamento via criptomoedas',
    'cryptoPaymentCreated': 'Pagamento via criptomoedas criado com sucesso!',
    'currencySymbol': 'R$',
    'paymentMethodPix': 'PIX',
    'paymentMethodCrypto': 'Criptomoedas',
    'qrCodePixAlt': 'QR Code PIX',
    // Landing Page
        'heroSubtitle': 'Gere contas premium para os melhores serviços',
        'heroDescription': 'Acesso rápido, seguro e confiável às plataformas mais populares',
        'heroBadge': 'PLATAFORMA #1 DO BRASIL',
        'heroTrustedBy': 'Confiado por mais de 25 mil criadores e revendedores',
    'startNow': 'Começar Agora',
    'viewPlans': 'Ver Planos',
        'metricsUsers': 'Usuários ativos',
        'metricsUsersDesc': 'Comunidade conectada diariamente',
        'metricsAccounts': 'Contas entregues',
        'metricsAccountsDesc': 'Distribuídas automaticamente',
        'metricsUptime': 'Uptime monitorado',
        'metricsUptimeDesc': 'Infraestrutura estável 24/7',
        'metricsSupport': 'Tempo médio de suporte',
        'metricsSupportDesc': '< 5 min via chat dedicado',
        'metricsLatency': 'Ativação média',
        'metricsCountries': 'Países atendidos',
        'partnersTitle': 'Escolhida por times que vivem de automação',
        'liveMonitorTitle': 'Monitoramento em tempo real',
        'liveMonitorDesc': 'Estoque, pagamentos e filas acompanhados 24/7 para você nunca parar.',
        'workflowTitle': 'Como funciona na prática',
        'workflowDesc': 'Em poucos cliques você ativa seu plano, gera credenciais e começa a vender.',
        'workflowStep1Title': 'Escolha seu plano',
        'workflowStep1Desc': 'Ativação instantânea via PIX ou criptomoedas.',
        'workflowStep2Title': 'Gere automaticamente',
        'workflowStep2Desc': 'Selecione o serviço, clique em gerar e receba credenciais premium.',
        'workflowStep3Title': 'Escale sem limites',
        'workflowStep3Desc': 'Compartilhe com sua base ou use para seus clientes com suporte contínuo.',
        'planDaily': 'Plano Diário',
        'planMonthly': 'Plano Mensal',
        'planLifetime': 'Plano Vitalício',
        'popupUserActivated': 'ativou',
        'popupJustNow': 'agora mesmo',
        'maintenanceSoftTitle': 'Modo manutenção ativado',
        'maintenanceSoftSubtitle': 'Estamos trabalhando em melhorias, mas você ainda pode fazer login normalmente.',
        'maintenanceTitle': 'Voltamos em instantes',
        'maintenanceSubtitle': 'Estamos aplicando melhorias importantes. Assim que finalizarmos você terá acesso novamente.',
        'maintenanceContact': 'Precisa de ajuda? Chame nosso suporte.',
        'maintenanceButton': 'Voltar ao início',
        'maintenanceStatus': 'Status em tempo real',
        'enterCoupon': 'Adicionar cupom',
        'couponPlaceholder': 'EX: PROMO50',
        'applyCoupon': 'Aplicar cupom',
        'removeCoupon': 'Remover cupom',
        'couponApplied': 'Cupom aplicado!',
        'invalidCoupon': 'Cupom inválido ou expirado',
        'discount': 'Desconto',
        'finalPrice': 'Valor final',
        'coupons': 'Cupons',
        'couponManager': 'Gerenciar Cupons',
        'createCoupon': 'Criar Cupom',
        'couponCode': 'Código do cupom',
        'couponDescription': 'Descrição',
        'couponDiscountType': 'Tipo de desconto',
        'couponDiscountValue': 'Valor do desconto',
        'couponMinAmount': 'Valor mínimo do pedido (opcional)',
        'couponMaxUses': 'Limite de uso',
        'couponExpiresAt': 'Data de expiração',
        'couponUses': 'Usos',
        'couponActive': 'Ativo',
        'couponInactive': 'Inativo',
        'couponTypePercentage': 'Porcentagem (%)',
        'couponTypeValue': 'Valor fixo (R$)',
        'couponCreatedSuccess': 'Cupom criado com sucesso!',
        'expirationSettings': 'Expiração da chave',
        'noExpiration': 'Sem expiração',
        'minutes': 'Minutos',
        'hours': 'Horas',
        'months': 'Meses',
        'years': 'Anos',
    'whyChooseUs': 'Por que escolher a gente?',
    'whyChooseUsDesc': 'Tecnologia avançada, segurança máxima e experiência premium',
    'fastInstant': 'Rápido e Instantâneo',
    'fastInstantDesc': 'Gere contas em segundos com nossa tecnologia de ponta. Sem espera, sem complicação.',
    'secure100': '100% Seguro',
    'secure100Desc': 'Criptografia de nível bancário e proteção avançada para garantir total segurança dos seus dados.',
    'multipleServices': 'Múltiplos Serviços',
    'multipleServicesDesc': 'Acesso a diversas plataformas populares com um único plano. Streaming, gaming e muito mais.',
    'premiumQuality': 'Premium Quality',
    'premiumQualityDesc': 'Contas de alta qualidade testadas e verificadas para garantir a melhor experiência.',
    'support247': 'Suporte 24/7',
    'support247Desc': 'Equipe especializada disponível sempre que precisar. Resolvemos qualquer questão rapidamente.',
    'freePlan': 'Plano Gratuito',
    'freePlanDesc': 'Comece com 2 gerações grátis por dia. Sem compromisso, sem cartão de crédito.',
    'whatClientsSay': 'O que nossos clientes dizem',
    'whatClientsSayDesc': 'Veja o que nossos usuários estão falando sobre nossa plataforma',
    'viewAllFeedbacks': 'Ver todos os feedbacks',
    'readyToStart': 'Pronto para começar?',
    'readyToStartDesc': 'Escolha um plano e tenha acesso imediato a todos os nossos serviços premium',
    'createFreeAccount': 'Criar Conta Grátis',
    // Layout Menu
    'affiliates': 'Afiliados',
    'raffles': 'Sorteios',
    'support': 'Suporte',
    'feedbacks': 'Feedbacks',
    'redeemKey': 'Resgatar Chave',
    'settings': 'Configurações',
    'chat': 'Chat',
    'tickets': 'Tickets',
    'administrator': 'Administrador',
    'user': 'Usuário',
    'allRightsReserved': 'Todos os direitos reservados',
    // Dashboard
    'welcome': 'Bem-vindo',
    'viewProfile': 'Ver Perfil',
    'expiresIn': 'Expira em',
    'durationLabel': 'Duração',
    'days': 'dias',
    'generationsLabel': 'Gerações',
    'unlimitedLabel': 'Ilimitadas',
    'freePlanLabel': 'Plano Free',
    'youAreUsingFreePlan': 'Você está usando o plano gratuito',
    'dailyGenerations': 'Gerações diárias',
    'free': 'grátis',
    'upgradeToPremium': 'Upgrade para Premium',
    'generating': 'Gerando...',
    'youDontHaveActivePlan': 'Você não possui um plano ativo',
    'youHave2FreeGenerations': '💡 Você tem 2 gerações grátis por dia!',
    'accountGeneratedSuccess': 'Conta Gerada com Sucesso! ✅',
    'accountFormat': 'Conta (formato: account:pass):',
    'copyFullAccount': 'Copiar conta completa',
    'emailUser': 'Email/Usuário:',
    'usernameLabel': 'Usuário:',
    'emailUserCopied': 'Email/Usuário copiado!',
    'passwordLabel': 'Senha:',
    'passwordCopied': 'Senha copiada!',
    'copyButton': '📋 Copiar',
    'importantInfo': 'ℹ️ Informação Importante:',
    'accountNotWorkingInfo': 'Se a conta não funcionar, não há problema! Você pode gerar novamente. Às vezes o estoque pode estar vencendo ou alguém pode ter trocado a senha.',
    'saveCredentials': '⚠️ Importante: Salve estas credenciais em um local seguro. Elas não serão exibidas novamente.',
    'yourAffiliateLink': 'Seu Link de Afiliado',
    'linkToShare': 'Link para compartilhar:',
    'copyLink': '📋 Copiar Link',
    'affiliateTip': '💡 Dica: Compartilhe este link com seus amigos! Quando eles se cadastrarem através do seu link, você ganha 2 gerações grátis e eles também ganham 2 gerações grátis!',
    'viewFullAffiliateStats': 'Ver estatísticas completas de afiliados →',
    'availableServices': 'Serviços Disponíveis',
    'availableLabel': 'disponíveis',
    'noServicesAvailable': 'Nenhum serviço disponível no momento.',
    'errorLoadingServices': 'Erro ao carregar serviços',
    'errorGeneratingAccount': 'Erro ao gerar conta',
    'affiliateLinkCopied': 'Link de afiliado copiado!',
    // Admin Dashboard
    'adminPanel': 'Painel Administrativo',
    'update': 'Atualizar',
    'totalUsers': 'Total de Usuários',
    'totalRevenue': 'Receita Total',
    'confirmedPayments': 'Pagamentos Confirmados',
    'availableStocks': 'Estoques Disponíveis',
    'activeServices': 'Serviços Ativos',
    'generatedAccounts': 'Contas Geradas',
    'availableKeys': 'Chaves Disponíveis',
    'quickActions': 'Ações Rápidas',
    'recentUsers': 'Usuários Recentes',
    'recentPayments': 'Pagamentos Recentes',
    'noEmail': 'Sem email',
    'noPlan': 'Sem plano',
    'noRecentUsers': 'Nenhum usuário recente',
    'noRecentPayments': 'Nenhum pagamento recente',
    'errorLoadingStats': 'Erro ao carregar estatísticas',
    'loading': 'Carregando...',
    'activatePlanAuto': 'Ativar plano automático',
    'activatingPlan': 'Ativando plano...',
    'planActivationPending': 'Plano ainda não foi ativado automaticamente',
    'planActivatedManualSuccess': 'Plano ativado com sucesso!',
    'errorActivatingPlan': 'Erro ao ativar plano',
    // Feedback Page
    'shareYourExperience': 'Compartilhe sua experiência conosco! Seu feedback ajuda a melhorar nossos serviços.',
    'sendFeedback': 'Enviar Feedback',
    'nameOrUsername': 'Nome (ou use seu username)',
    'yourName': 'Seu nome',
    'ratingOptional': 'Avaliação (opcional)',
    'message': 'Mensagem *',
    'shareExperiencePlaceholder': 'Compartilhe sua experiência, sugestões ou elogios...',
    'charactersCount': 'caracteres (mínimo 10)',
    'sending': 'Enviando...',
    'sendFeedbackButton': 'Enviar Feedback',
    'feedbackWillBeReviewed': '⚠️ Seu feedback será revisado antes de ser publicado',
    'approvedFeedbacks': 'Feedbacks Aprovados',
    'noFeedbackYet': 'Nenhum feedback ainda',
    'beFirstToShare': 'Seja o primeiro a compartilhar sua experiência!',
    'errorLoadingFeedbacks': 'Erro ao carregar feedbacks',
    'pleaseFillAllFields': 'Por favor, preencha todos os campos obrigatórios',
    'messageMinLength': 'A mensagem deve ter pelo menos 10 caracteres',
    'feedbackSentSuccess': 'Feedback enviado com sucesso! Aguarde aprovação do administrador.',
    'errorSendingFeedback': 'Erro ao enviar feedback',
    // Login
    'enterYourAccount': 'Entre na sua conta para continuar',
    'enterUsername': 'Digite seu usuário',
    'enterPassword': 'Digite sua senha',
    'loggingIn': 'Entrando...',
    'invalidCredentials': 'Credenciais inválidas',
    'loginSuccess': 'Login realizado com sucesso!',
    'errorLoggingIn': 'Erro ao fazer login',
    'dontHaveAccount': 'Não tem uma conta?',
    'backToHome': '← Voltar para a página inicial',
    // Register
    'joinUs': 'Junte-se a nós e comece agora',
    'youWillGet2FreeGenerations': '🎁 Você ganhará 2 gerações grátis ao se cadastrar através deste link!',
    'emailOptional': '(Opcional)',
    'enterEmail': 'Digite seu email',
    'confirmPassword': 'Confirmar Senha',
    'enterConfirmPassword': 'Digite novamente sua senha',
    'passwordsDontMatch': 'As senhas não coincidem',
    'passwordMinLength': 'A senha deve ter pelo menos 6 caracteres',
    'accountCreatedSuccess': 'Conta criada com sucesso!',
    'accountCreatedButLoginError': 'Conta criada, mas erro ao fazer login automático. Faça login manualmente.',
    'errorCreatingAccount': 'Erro ao criar conta',
    'checkMongoDB': 'Verifique se o MongoDB está acessível e a DATABASE_URL está correta no .env',
    'configureMongoDB': 'Configure o MongoDB no arquivo .env e execute: npm run db:push',
    // Tickets
    'errorLoadingTickets': 'Erro ao carregar tickets',
    'ticketCreatedSuccess': 'Ticket criado com sucesso!',
    'errorCreatingTicket': 'Erro ao criar ticket',
    'enterMessage': 'Digite uma mensagem',
    'replySent': 'Resposta enviada!',
    'errorSendingReply': 'Erro ao enviar resposta',
    'errorLoadingTicketDetails': 'Erro ao carregar detalhes do ticket',
    'open': 'Aberto',
    'inProgress': 'Em Progresso',
    'resolved': 'Resolvido',
    'closed': 'Fechado',
    'low': 'Baixa',
    'medium': 'Média',
    'high': 'Alta',
    'urgent': 'Urgente',
    'subject': 'Assunto',
    'priority': 'Prioridade',
    'createTicket': 'Criar Ticket',
    'newTicket': 'Novo Ticket',
    'myTickets': 'Meus Tickets',
    'noTickets': 'Nenhum ticket encontrado',
    'createFirstTicket': 'Crie seu primeiro ticket de suporte',
    'reply': 'Responder',
    'sendReply': 'Enviar Resposta',
    'replies': 'Respostas',
    'noReplies': 'Nenhuma resposta ainda',
    'creating': 'Criando conta...',
    'alreadyHaveAccount': 'Já tem uma conta?',
    'selectLanguage': 'Selecione seu idioma',
    'selectLanguageDescription': 'Escolha o idioma que você fala para que possamos atendê-lo melhor no Telegram.',
    'yourLanguage': 'Seu idioma',
    'otherLanguage': 'Outro idioma',
    'payManualInternational': 'Pagamento Manual (Internacional)',
    'manualPaymentInternational': 'Pagamento Manual Internacional',
    'manualPaymentDescription': 'Para usuários que não são do Brasil, oferecemos pagamento manual via Telegram. Aceitamos Wise e qualquer criptomoeda.',
    'acceptedPaymentMethods': 'Métodos de pagamento aceitos:',
    'wiseDescription': 'Transferência bancária internacional via Wise',
    'anyCryptocurrency': 'Qualquer Criptomoeda',
    'cryptoDescription': 'Bitcoin, Ethereum, USDT e outras criptomoedas',
    'manualPaymentInstructions': 'Clique no botão abaixo para abrir o Telegram. Envie uma mensagem informando o plano desejado e o método de pagamento que prefere usar. Responderemos rapidamente com as instruções de pagamento.'
  },
  'en': {
    'siteName': 'Kaizen Gens',
    'login': 'Login',
    'logout': 'Logout',
    'register': 'Register',
    'createAccount': 'Create Account',
    'username': 'Username',
    'password': 'Password',
    'email': 'Email',
    'admin': 'Admin',
    'dashboard': 'Dashboard',
    'services': 'Services',
    'stocks': 'Stocks',
    'plans': 'Plans',
    'keys': 'Keys',
    'users': 'Users',
    'generate': 'Generate',
    'account': 'Account',
    'accounts': 'Accounts',
    'create': 'Create',
    'edit': 'Edit',
    'delete': 'Delete',
    'save': 'Save',
    'cancel': 'Cancel',
    'name': 'Name',
    'description': 'Description',
    'price': 'Price',
    'duration': 'Duration (days)',
    'active': 'Active',
    'inactive': 'Inactive',
    'status': 'Status',
    'payment': 'Payment',
    'pay': 'Pay',
    'pix': 'PIX',
    'bitcoin': 'Bitcoin',
    'pending': 'Pending',
    'paid': 'Paid',
    'expired': 'Expired',
    'myPlan': 'My Plan',
    'expiresAt': 'Expires at',
    'generationsLeft': 'Generations left',
    'unlimited': 'Unlimited',
    'selectService': 'Select a service',
    'generateAccount': 'Generate Account',
    'available': 'Available',
    'unavailable': 'Unavailable',
    'add': 'Add',
    'search': 'Search',
    'noResults': 'No results found',
    'confirm': 'Confirm',
    'areYouSure': 'Are you sure?',
    'createdAt': 'Created at',
    'updatedAt': 'Updated at',
    'plansDescription': 'Choose the perfect plan for you and get access to all our premium services',
    'mostPopular': 'MOST POPULAR',
    'daysAccess': 'days of access',
    'generations': 'generations',
    'accessAllServices': 'Access to all services',
    'payViaPix': 'Pay via PIX',
    'payViaCrypto': 'Pay via Cryptocurrencies',
    'payViaCard': 'Pay via Credit Card',
    'comingSoon': 'Coming soon',
    'cardPaymentDescription': 'Enter your credit card information securely. Your payment is processed directly by PagBank with bank-level encryption.',
    'cardNumber': 'Card Number',
    'expMonth': 'Month',
    'expYear': 'Year',
    'cardHolderName': 'Cardholder Name',
    'cardHolderNamePlaceholder': 'NAME AS ON CARD',
    'cardSecurityNotice': 'Your data is protected with SSL encryption.',
    'cardSecurityNoticeTokenized': 'Your card data is processed directly by PagBank. No sensitive data is stored on our servers.',
    'securePayment': 'Secure Payment',
    'pciCompliant': 'PCI Compliant',
    'sslEncrypted': 'SSL Encrypted',
    'pagbankSecure': 'Processed by PagBank',
    'tokenizingCard': 'Tokenizing card...',
    'cardTokenized': 'Card tokenized successfully',
    'tokenizationError': 'Error tokenizing card. Please try again.',
    'payNow': 'Pay Now',
    'processing': 'Processing...',
    'cardPaymentApproved': 'Payment approved! Your plan has been activated.',
    'cardPaymentPending': 'Payment under review. Please wait for confirmation.',
    'cardPaymentError': 'Error processing payment. Please try again.',
    'invalidCardNumber': 'Invalid card number.',
    'invalidExpMonth': 'Invalid expiration month.',
    'invalidExpYear': 'Invalid expiration year.',
    'invalidCvv': 'Invalid CVV.',
    'invalidCardHolderName': 'Invalid cardholder name.',
    'paymentMethodCard': 'Credit Card',
    'noPlansAvailable': 'No plans available at the moment.',
    'informEmail': 'Enter your email',
    'emailRequired': 'A valid email is required to process PIX payments. This email will only be used for transaction purposes and will not be stored.',
    'emailLabel': 'Email *',
    'emailPlaceholder': 'your@email.com',
    'continue': 'Continue',
    'close': 'Close',
    'paymentVia': 'Payment via',
    'creatingPixPayment': 'Creating PIX payment...',
    'checkingPayment': 'Checking payment automatically... (every 5 seconds)',
    'pixCodeCopyPaste': 'PIX Code (Copy and Paste)',
    'instructions': 'Instructions:',
    'instructionsText': 'Scan the QR Code with your banking app or copy and paste the PIX code to complete the payment.',
    'autoActivation': '✨ Automatic Activation:',
    'autoActivationText': 'Our system automatically checks for payment confirmation every 5 seconds. Once your payment is confirmed, your plan will be activated immediately!',
    'waitProcessing': 'Please wait:',
    'waitProcessingText': 'We are processing PIX payment data. Please reload the page in a few moments or check the payment code in your account.',
    'amountToPay': 'Amount to pay:',
    'bitcoinAddress': 'Bitcoin Address',
    'copy': 'Copy',
    'network': 'Network:',
    'sendExactly': 'Send exactly',
    'toAddressAbove': 'to the address above.',
    'paymentProcessed': 'Payment will be processed automatically after network confirmation.',
    'important': '⚠️ Important:',
    'importantText': 'Please double-check the address before sending. Cryptocurrency transactions are irreversible and cannot be refunded.',
    'attention': 'Attention:',
    'autoPaymentUnavailable': 'The automatic payment system is temporarily unavailable.',
    'contactTelegram': 'Please contact us via Telegram to complete your payment:',
    'openTelegram': 'Open Telegram',
    'joinTelegram': 'Join Telegram',
    'joinDiscord': 'Join Discord',
    'freeKeys': 'Join our Telegram and Discord communities! We regularly release free access keys to the generator and much more exclusive content.',
    'or': 'or',
    'loadingQrCode': 'Loading QR code...',
    'pixPaymentCreated': 'PIX payment created successfully!',
    'paymentConfirmed': '🎉 Payment confirmed! Your plan has been activated automatically!',
    'loginToContinue': 'Please login to continue',
    'errorLoadingPlans': 'Error loading plans',
    'errorCreatingPix': 'Error creating PIX payment',
    'errorCreatingPayment': 'An error occurred while creating the payment. Please try again or contact support if the problem persists.',
    'errorInvalidEmail': 'Please enter a valid email',
    'errorIncompletePaymentData': 'Error: Payment data is incomplete. Please try again or contact support for assistance.',
    'addressCopied': 'Address copied!',
    'errorCreatingPaymentCrypto': 'Error creating cryptocurrency payment',
    'cryptoPaymentCreated': 'Cryptocurrency payment created successfully!',
    'currencySymbol': '$',
    'paymentMethodPix': 'PIX',
    'paymentMethodCrypto': 'Cryptocurrencies',
    'qrCodePixAlt': 'PIX QR Code',
    // Landing Page
        'heroSubtitle': 'Generate premium accounts for the best services',
        'heroDescription': 'Fast, secure and reliable access to the most popular platforms',
        'heroBadge': '#1 PLATFORM IN BRAZIL',
        'heroTrustedBy': 'Trusted by 25k+ creators and resellers',
    'startNow': 'Start Now',
    'viewPlans': 'View Plans',
        'metricsUsers': 'Active users',
        'metricsUsersDesc': 'People connected daily',
        'metricsAccounts': 'Accounts delivered',
        'metricsAccountsDesc': 'Sent automatically',
        'metricsUptime': 'Monitored uptime',
        'metricsUptimeDesc': 'Stable infrastructure 24/7',
        'metricsSupport': 'Avg. support time',
        'metricsSupportDesc': '< 5 min on dedicated chat',
        'metricsLatency': 'Average activation',
        'metricsCountries': 'Countries served',
        'partnersTitle': 'Chosen by teams that live on automation',
        'liveMonitorTitle': 'Real-time monitoring',
        'liveMonitorDesc': 'Inventory, payments and queues tracked 24/7 so you never stop.',
        'workflowTitle': 'How it works in practice',
        'workflowDesc': 'In a few clicks you activate your plan, generate credentials and start selling.',
        'workflowStep1Title': 'Pick your plan',
        'workflowStep1Desc': 'Instant activation via PIX or crypto.',
        'workflowStep2Title': 'Generate automatically',
        'workflowStep2Desc': 'Select the service, click generate and receive premium credentials.',
        'workflowStep3Title': 'Scale without limits',
        'workflowStep3Desc': 'Share with your audience or deliver to clients with continuous support.',
        'planDaily': 'Daily Plan',
        'planMonthly': 'Monthly Plan',
        'planLifetime': 'Lifetime Plan',
        'popupUserActivated': 'activated',
        'popupJustNow': 'just now',
        'maintenanceSoftTitle': 'Maintenance mode on',
        'maintenanceSoftSubtitle': 'We are shipping new features, but you can still log in normally.',
        'maintenanceTitle': 'We will be back shortly',
        'maintenanceSubtitle': 'We are applying important upgrades. As soon as we finish you will regain access automatically.',
        'maintenanceContact': 'Need help? Talk to our support team.',
        'maintenanceButton': 'Back to home',
        'maintenanceStatus': 'Live status',
        'enterCoupon': 'Add coupon',
        'couponPlaceholder': 'E.g. SAVE50',
        'applyCoupon': 'Apply coupon',
        'removeCoupon': 'Remove coupon',
        'couponApplied': 'Coupon applied!',
        'invalidCoupon': 'Invalid or expired coupon',
        'discount': 'Discount',
        'finalPrice': 'Final price',
        'coupons': 'Coupons',
        'couponManager': 'Coupon Manager',
        'createCoupon': 'Create Coupon',
        'couponCode': 'Coupon code',
        'couponDescription': 'Description',
        'couponDiscountType': 'Discount type',
        'couponDiscountValue': 'Discount value',
        'couponMinAmount': 'Minimum order amount (optional)',
        'couponMaxUses': 'Usage limit',
        'couponExpiresAt': 'Expires at',
        'couponUses': 'Uses',
        'couponActive': 'Active',
        'couponInactive': 'Inactive',
        'couponTypePercentage': 'Percentage (%)',
        'couponTypeValue': 'Fixed amount ($)',
        'couponCreatedSuccess': 'Coupon created successfully!',
        'expirationSettings': 'Key expiration',
        'noExpiration': 'No expiration',
        'minutes': 'Minutes',
        'hours': 'Hours',
        'months': 'Months',
        'years': 'Years',
    'whyChooseUs': 'Why choose us?',
    'whyChooseUsDesc': 'Advanced technology, maximum security and premium experience',
    'fastInstant': 'Fast and Instant',
    'fastInstantDesc': 'Generate accounts in seconds with our cutting-edge technology. No waiting, no complications.',
    'secure100': '100% Secure',
    'secure100Desc': 'Bank-level encryption and advanced protection to ensure total security of your data.',
    'multipleServices': 'Multiple Services',
    'multipleServicesDesc': 'Access to various popular platforms with a single plan. Streaming, gaming and much more.',
    'premiumQuality': 'Premium Quality',
    'premiumQualityDesc': 'High-quality accounts tested and verified to ensure the best experience.',
    'support247': '24/7 Support',
    'support247Desc': 'Specialized team available whenever you need. We solve any issue quickly.',
    'freePlan': 'Free Plan',
    'freePlanDesc': 'Start with 2 free generations per day. No commitment, no credit card.',
    'whatClientsSay': 'What our clients say',
    'whatClientsSayDesc': 'See what our users are saying about our platform',
    'viewAllFeedbacks': 'View all feedbacks',
    'readyToStart': 'Ready to start?',
    'readyToStartDesc': 'Choose a plan and get immediate access to all our premium services',
    'createFreeAccount': 'Create Free Account',
    // Layout Menu
    'affiliates': 'Affiliates',
    'raffles': 'Raffles',
    'support': 'Support',
    'feedbacks': 'Feedbacks',
    'redeemKey': 'Redeem Key',
    'settings': 'Settings',
    'chat': 'Chat',
    'tickets': 'Tickets',
    'administrator': 'Administrator',
    'user': 'User',
    'allRightsReserved': 'All rights reserved',
    // Dashboard
    'welcome': 'Welcome',
    'viewProfile': 'View Profile',
    'expiresIn': 'Expires in',
    'durationLabel': 'Duration',
    'days': 'days',
    'generationsLabel': 'Generations',
    'unlimitedLabel': 'Unlimited',
    'freePlanLabel': 'Free Plan',
    'youAreUsingFreePlan': 'You are using the free plan',
    'dailyGenerations': 'Daily generations',
        'free': 'free',
        'upgradeToPremium': 'Upgrade to Premium',
    'generating': 'Generating...',
    'youDontHaveActivePlan': 'You do not have an active plan',
    'youHave2FreeGenerations': '💡 You have 2 free generations per day!',
    'accountGeneratedSuccess': 'Account Generated Successfully! ✅',
    'accountFormat': 'Account (format: account:pass):',
    'copyFullAccount': 'Copy full account',
    'emailUser': 'Email/Username:',
    'usernameLabel': 'Username:',
    'emailUserCopied': 'Email/Username copied!',
    'passwordLabel': 'Password:',
    'passwordCopied': 'Password copied!',
    'copyButton': '📋 Copy',
    'importantInfo': 'ℹ️ Important Information:',
    'accountNotWorkingInfo': 'If the account does not work, no problem! You can generate again. Sometimes the stock may be expiring or someone may have changed the password.',
    'saveCredentials': '⚠️ Important: Save these credentials in a safe place. They will not be displayed again.',
    'yourAffiliateLink': 'Your Affiliate Link',
    'linkToShare': 'Link to share:',
    'copyLink': '📋 Copy Link',
    'affiliateTip': '💡 Tip: Share this link with your friends! When they register through your link, you get 2 free generations and they also get 2 free generations!',
    'viewFullAffiliateStats': 'View full affiliate statistics →',
    'availableServices': 'Available Services',
    'availableLabel': 'available',
    'noServicesAvailable': 'No services available at the moment.',
    'errorLoadingServices': 'Error loading services',
    'errorGeneratingAccount': 'Error generating account',
    'affiliateLinkCopied': 'Affiliate link copied!',
    // Admin Dashboard
    'adminPanel': 'Administrative Panel',
    'update': 'Update',
    'totalUsers': 'Total Users',
    'totalRevenue': 'Total Revenue',
    'confirmedPayments': 'Confirmed Payments',
    'availableStocks': 'Available Stocks',
    'activeServices': 'Active Services',
    'generatedAccounts': 'Generated Accounts',
    'availableKeys': 'Available Keys',
    'quickActions': 'Quick Actions',
    'recentUsers': 'Recent Users',
    'recentPayments': 'Recent Payments',
    'noEmail': 'No email',
    'noPlan': 'No plan',
    'noRecentUsers': 'No recent users',
    'noRecentPayments': 'No recent payments',
    'errorLoadingStats': 'Error loading statistics',
    'loading': 'Loading...',
    'activatePlanAuto': 'Activate plan automatically',
    'activatingPlan': 'Activating plan...',
    'planActivationPending': 'Plan has not been activated automatically yet',
    'planActivatedManualSuccess': 'Plan activated successfully!',
    'errorActivatingPlan': 'Error activating plan',
    // Feedback Page
    'shareYourExperience': 'Share your experience with us! Your feedback helps improve our services.',
    'sendFeedback': 'Send Feedback',
    'nameOrUsername': 'Name (or use your username)',
    'yourName': 'Your name',
    'ratingOptional': 'Rating (optional)',
    'message': 'Message *',
    'shareExperiencePlaceholder': 'Share your experience, suggestions or compliments...',
    'charactersCount': 'characters (minimum 10)',
    'sending': 'Sending...',
    'sendFeedbackButton': 'Send Feedback',
    'feedbackWillBeReviewed': '⚠️ Your feedback will be reviewed before being published',
    'approvedFeedbacks': 'Approved Feedbacks',
    'noFeedbackYet': 'No feedback yet',
    'beFirstToShare': 'Be the first to share your experience!',
    'errorLoadingFeedbacks': 'Error loading feedbacks',
    'pleaseFillAllFields': 'Please fill in all required fields',
    'messageMinLength': 'The message must be at least 10 characters',
    'feedbackSentSuccess': 'Feedback sent successfully! Wait for administrator approval.',
    'errorSendingFeedback': 'Error sending feedback',
    // Login
    'enterYourAccount': 'Enter your account to continue',
    'enterUsername': 'Enter your username',
    'enterPassword': 'Enter your password',
    'loggingIn': 'Logging in...',
    'invalidCredentials': 'Invalid credentials',
    'loginSuccess': 'Login successful!',
    'errorLoggingIn': 'Error logging in',
    'dontHaveAccount': "Don't have an account?",
    'backToHome': '← Back to home page',
    // Register
    'joinUs': 'Join us and start now',
    'youWillGet2FreeGenerations': '🎁 You will get 2 free generations when you register through this link!',
    'emailOptional': '(Optional)',
    'enterEmail': 'Enter your email',
    'confirmPassword': 'Confirm Password',
    'enterConfirmPassword': 'Enter your password again',
    'passwordsDontMatch': 'Passwords do not match',
    'passwordMinLength': 'Password must be at least 6 characters',
    'accountCreatedSuccess': 'Account created successfully!',
    'accountCreatedButLoginError': 'Account created, but error during automatic login. Please login manually.',
    'errorCreatingAccount': 'Error creating account',
    'checkMongoDB': 'Check if MongoDB is accessible and DATABASE_URL is correct in .env',
    'configureMongoDB': 'Configure MongoDB in .env file and run: npm run db:push',
    // Tickets
    'errorLoadingTickets': 'Error loading tickets',
    'ticketCreatedSuccess': 'Ticket created successfully!',
    'errorCreatingTicket': 'Error creating ticket',
    'enterMessage': 'Enter a message',
    'replySent': 'Reply sent!',
    'errorSendingReply': 'Error sending reply',
    'errorLoadingTicketDetails': 'Error loading ticket details',
    'open': 'Open',
    'inProgress': 'In Progress',
    'resolved': 'Resolved',
    'closed': 'Closed',
    'low': 'Low',
    'medium': 'Medium',
    'high': 'High',
    'urgent': 'Urgent',
    'subject': 'Subject',
    'priority': 'Priority',
    'createTicket': 'Create Ticket',
    'newTicket': 'New Ticket',
    'myTickets': 'My Tickets',
    'noTickets': 'No tickets found',
    'createFirstTicket': 'Create your first support ticket',
    'reply': 'Reply',
    'sendReply': 'Send Reply',
    'replies': 'Replies',
    'noReplies': 'No replies yet',
    'creating': 'Creating account...',
    'alreadyHaveAccount': 'Already have an account?',
    'selectLanguage': 'Select your language',
    'selectLanguageDescription': 'Choose the language you speak so we can assist you better on Telegram.',
    'yourLanguage': 'Your language',
    'otherLanguage': 'Other language',
    'payManualInternational': 'Manual Payment (International)',
    'manualPaymentInternational': 'International Manual Payment',
    'manualPaymentDescription': 'For users outside Brazil, we offer manual payment via Telegram. We accept Wise and any cryptocurrency.',
    'acceptedPaymentMethods': 'Accepted payment methods:',
    'wiseDescription': 'International bank transfer via Wise',
    'anyCryptocurrency': 'Any Cryptocurrency',
    'cryptoDescription': 'Bitcoin, Ethereum, USDT and other cryptocurrencies',
    'manualPaymentInstructions': 'Click the button below to open Telegram. Send a message informing the desired plan and the payment method you prefer to use. We will respond quickly with payment instructions.'
  },
  'es': {
    'siteName': 'Kaizen Gens',
    'login': 'Iniciar sesión',
    'logout': 'Cerrar sesión',
    'register': 'Registrarse',
    'createAccount': 'Crear Cuenta',
    'username': 'Usuario',
    'password': 'Contraseña',
    'email': 'Email',
    'admin': 'Admin',
    'dashboard': 'Panel',
    'services': 'Servicios',
    'stocks': 'Inventario',
    'plans': 'Planes',
    'keys': 'Claves',
    'users': 'Usuarios',
    'generate': 'Generar',
    'account': 'Cuenta',
    'accounts': 'Cuentas',
    'create': 'Crear',
    'edit': 'Editar',
    'delete': 'Eliminar',
    'save': 'Guardar',
    'cancel': 'Cancelar',
    'name': 'Nombre',
    'description': 'Descripción',
    'price': 'Precio',
    'duration': 'Duración (días)',
    'active': 'Activo',
    'inactive': 'Inactivo',
    'status': 'Estado',
    'payment': 'Pago',
    'pay': 'Pagar',
    'pix': 'PIX',
    'bitcoin': 'Bitcoin',
    'pending': 'Pendiente',
    'paid': 'Pagado',
    'expired': 'Expirado',
    'myPlan': 'Mi Plan',
    'expiresAt': 'Expira en',
    'generationsLeft': 'Generaciones restantes',
    'unlimited': 'Ilimitado',
    'selectService': 'Selecciona un servicio',
    'generateAccount': 'Generar Cuenta',
    'available': 'Disponible',
    'unavailable': 'No disponible',
    'add': 'Agregar',
    'search': 'Buscar',
    'noResults': 'No se encontraron resultados',
    'confirm': 'Confirmar',
    'areYouSure': '¿Estás seguro?',
    'createdAt': 'Creado en',
    'updatedAt': 'Actualizado en',
    'plansDescription': 'Elige el plan perfecto para ti y obtén acceso a todos nuestros servicios premium',
    'mostPopular': 'MÁS POPULAR',
    'daysAccess': 'días de acceso',
    'generations': 'generaciones',
    'accessAllServices': 'Acceso a todos los servicios',
    'payViaPix': 'Pagar vía PIX',
    'payViaCrypto': 'Pagar vía Criptomonedas',
    'payViaCard': 'Pagar vía Tarjeta',
    'comingSoon': 'Próximamente',
    'cardPaymentDescription': 'Ingresa la información de tu tarjeta de crédito de forma segura. Tu pago es procesado directamente por PagBank con encriptación de nivel bancario.',
    'cardNumber': 'Número de Tarjeta',
    'expMonth': 'Mes',
    'expYear': 'Año',
    'cardHolderName': 'Nombre del Titular',
    'cardHolderNamePlaceholder': 'NOMBRE COMO EN LA TARJETA',
    'cardSecurityNotice': 'Tus datos están protegidos con encriptación SSL.',
    'cardSecurityNoticeTokenized': 'Los datos de tu tarjeta son procesados directamente por PagBank. No se almacenan datos sensibles en nuestros servidores.',
    'securePayment': 'Pago Seguro',
    'pciCompliant': 'Cumple con PCI',
    'sslEncrypted': 'Encriptado SSL',
    'pagbankSecure': 'Procesado por PagBank',
    'tokenizingCard': 'Tokenizando tarjeta...',
    'cardTokenized': 'Tarjeta tokenizada con éxito',
    'tokenizationError': 'Error al tokenizar tarjeta. Por favor, intenta de nuevo.',
    'payNow': 'Pagar Ahora',
    'processing': 'Procesando...',
    'cardPaymentApproved': '¡Pago aprobado! Tu plan ha sido activado.',
    'cardPaymentPending': 'Pago en revisión. Por favor, espera la confirmación.',
    'cardPaymentError': 'Error al procesar el pago. Por favor, intenta de nuevo.',
    'invalidCardNumber': 'Número de tarjeta inválido.',
    'invalidExpMonth': 'Mes de vencimiento inválido.',
    'invalidExpYear': 'Año de vencimiento inválido.',
    'invalidCvv': 'CVV inválido.',
    'invalidCardHolderName': 'Nombre del titular inválido.',
    'paymentMethodCard': 'Tarjeta de Crédito',
    'noPlansAvailable': 'No hay planes disponibles en este momento.',
    'informEmail': 'Ingresa tu email',
    'emailRequired': 'Se requiere un email válido para procesar pagos PIX. Este email solo se usará para fines de transacción y no se almacenará.',
    'emailLabel': 'Email *',
    'emailPlaceholder': 'tu@email.com',
    'continue': 'Continuar',
    'close': 'Cerrar',
    'paymentVia': 'Pago vía',
    'creatingPixPayment': 'Creando pago PIX...',
    'checkingPayment': 'Verificando pago automáticamente... (cada 5 segundos)',
    'pixCodeCopyPaste': 'Código PIX (Copiar y Pegar)',
    'instructions': 'Instrucciones:',
    'instructionsText': 'Escanea el código QR con tu aplicación bancaria o copia y pega el código PIX para completar el pago.',
    'autoActivation': '✨ Activación Automática:',
    'autoActivationText': 'Nuestro sistema verifica automáticamente la confirmación del pago cada 5 segundos. Una vez que tu pago sea confirmado, tu plan será activado inmediatamente!',
    'waitProcessing': 'Por favor espera:',
    'waitProcessingText': 'Estamos procesando los datos del pago PIX. Por favor, recarga la página en unos momentos o verifica el código de pago en tu cuenta.',
    'amountToPay': 'Cantidad a pagar:',
    'bitcoinAddress': 'Dirección Bitcoin',
    'copy': 'Copiar',
    'network': 'Red:',
    'sendExactly': 'Envía exactamente',
    'toAddressAbove': 'a la dirección de arriba.',
    'paymentProcessed': 'El pago será procesado automáticamente después de la confirmación en la red.',
    'important': '⚠️ Importante:',
    'importantText': 'Por favor, verifica la dirección antes de enviar. Las transacciones de criptomonedas son irreversibles y no se pueden reembolsar.',
    'attention': 'Atención:',
    'autoPaymentUnavailable': 'El sistema de pago automático está temporalmente no disponible.',
    'contactTelegram': 'Por favor, contáctanos vía Telegram para completar tu pago:',
    'openTelegram': 'Abrir Telegram',
    'joinTelegram': 'Unirse a Telegram',
    'joinDiscord': 'Unirse a Discord',
    'freeKeys': '¡Únete a nuestras comunidades de Telegram y Discord! Regularmente lanzamos claves de acceso gratuitas al generador y mucho más contenido exclusivo.',
    'or': 'o',
    'loadingQrCode': 'Cargando código QR...',
    'pixPaymentCreated': '¡Pago PIX creado con éxito!',
    'paymentConfirmed': '🎉 ¡Pago confirmado! Tu plan ha sido activado automáticamente!',
    'loginToContinue': 'Por favor, inicia sesión para continuar',
    'errorLoadingPlans': 'Error al cargar planes',
    'errorCreatingPix': 'Error al crear pago PIX',
    'errorCreatingPayment': 'Ocurrió un error al crear el pago. Por favor, intenta de nuevo o contacta soporte si el problema persiste.',
    'errorInvalidEmail': 'Por favor, ingresa un email válido',
    'errorIncompletePaymentData': 'Error: Los datos del pago están incompletos. Por favor, intenta de nuevo o contacta soporte para asistencia.',
    'addressCopied': '¡Dirección copiada!',
    'errorCreatingPaymentCrypto': 'Error al crear pago con criptomonedas',
    'cryptoPaymentCreated': '¡Pago con criptomonedas creado con éxito!',
    'currencySymbol': '$',
    'paymentMethodPix': 'PIX',
    'paymentMethodCrypto': 'Criptomonedas',
    'qrCodePixAlt': 'Código QR PIX',
    'heroSubtitle': 'Genera cuentas premium para los mejores servicios',
    'heroDescription': 'Acceso rápido, seguro y confiable a las plataformas más populares',
    'heroBadge': '#1 PLATAFORMA EN BRASIL',
    'heroTrustedBy': 'Confiado por más de 25 mil creadores y revendedores',
    'startNow': 'Comenzar Ahora',
    'viewPlans': 'Ver Planes',
    'metricsUsers': 'Usuarios activos',
    'metricsUsersDesc': 'Personas conectadas diariamente',
    'metricsAccounts': 'Cuentas entregadas',
    'metricsAccountsDesc': 'Enviadas automáticamente',
    'metricsUptime': 'Tiempo de actividad monitoreado',
    'metricsUptimeDesc': 'Infraestructura estable 24/7',
    'metricsSupport': 'Tiempo promedio de soporte',
    'metricsSupportDesc': '< 5 min en chat dedicado',
    'metricsLatency': 'Activación promedio',
    'metricsCountries': 'Países atendidos',
    'partnersTitle': 'Elegido por equipos que viven de automatización',
    'liveMonitorTitle': 'Monitoreo en tiempo real',
    'liveMonitorDesc': 'Inventario, pagos y colas rastreadas 24/7 para que nunca te detengas.',
    'workflowTitle': 'Cómo funciona en la práctica',
    'workflowDesc': 'En pocos clics activas tu plan, generas credenciales y comienzas a vender.',
    'workflowStep1Title': 'Elige tu plan',
    'workflowStep1Desc': 'Activación instantánea vía PIX o cripto.',
    'workflowStep2Title': 'Genera automáticamente',
    'workflowStep2Desc': 'Selecciona el servicio, haz clic en generar y recibe credenciales premium.',
    'workflowStep3Title': 'Escala sin límites',
    'workflowStep3Desc': 'Comparte con tu audiencia o entrega a clientes con soporte continuo.',
    'planDaily': 'Plan Diario',
    'planMonthly': 'Plan Mensual',
    'planLifetime': 'Plan Vitalicio',
    'popupUserActivated': 'activó',
    'popupJustNow': 'ahora mismo',
    'maintenanceSoftTitle': 'Modo mantenimiento activado',
    'maintenanceSoftSubtitle': 'Estamos trabajando en mejoras, pero aún puedes iniciar sesión normalmente.',
    'maintenanceTitle': 'Volveremos en un momento',
    'maintenanceSubtitle': 'Estamos aplicando mejoras importantes. Tan pronto como terminemos tendrás acceso nuevamente.',
    'maintenanceContact': '¿Necesitas ayuda? Llama a nuestro soporte.',
    'maintenanceButton': 'Volver al inicio',
    'maintenanceStatus': 'Estado en tiempo real',
    'enterCoupon': 'Agregar cupón',
    'couponPlaceholder': 'Ej: PROMO50',
    'applyCoupon': 'Aplicar cupón',
    'removeCoupon': 'Quitar cupón',
    'couponApplied': '¡Cupón aplicado!',
    'invalidCoupon': 'Cupón inválido o expirado',
    'discount': 'Descuento',
    'finalPrice': 'Precio final',
    'coupons': 'Cupones',
    'couponManager': 'Gestor de Cupones',
    'createCoupon': 'Crear Cupón',
    'couponCode': 'Código del cupón',
    'couponDescription': 'Descripción',
    'couponDiscountType': 'Tipo de descuento',
    'couponDiscountValue': 'Valor del descuento',
    'couponMinAmount': 'Cantidad mínima del pedido (opcional)',
    'couponMaxUses': 'Límite de uso',
    'couponExpiresAt': 'Expira en',
    'couponUses': 'Usos',
    'couponActive': 'Activo',
    'couponInactive': 'Inactivo',
    'couponTypePercentage': 'Porcentaje (%)',
    'couponTypeValue': 'Cantidad fija ($)',
    'couponCreatedSuccess': '¡Cupón creado con éxito!',
    'expirationSettings': 'Expiración de la clave',
    'noExpiration': 'Sin expiración',
    'minutes': 'Minutos',
    'hours': 'Horas',
    'months': 'Meses',
    'years': 'Años',
    'whyChooseUs': '¿Por qué elegirnos?',
    'whyChooseUsDesc': 'Tecnología avanzada, máxima seguridad y experiencia premium',
    'fastInstant': 'Rápido e Instantáneo',
    'fastInstantDesc': 'Genera cuentas en segundos con nuestra tecnología de vanguardia. Sin espera, sin complicaciones.',
    'secure100': '100% Seguro',
    'secure100Desc': 'Encriptación de nivel bancario y protección avanzada para garantizar la seguridad total de tus datos.',
    'multipleServices': 'Múltiples Servicios',
    'multipleServicesDesc': 'Acceso a varias plataformas populares con un solo plan. Streaming, gaming y mucho más.',
    'premiumQuality': 'Calidad Premium',
    'premiumQualityDesc': 'Cuentas de alta calidad probadas y verificadas para garantizar la mejor experiencia.',
    'support247': 'Soporte 24/7',
    'support247Desc': 'Equipo especializado disponible cuando lo necesites. Resolvemos cualquier problema rápidamente.',
    'freePlan': 'Plan Gratuito',
    'freePlanDesc': 'Comienza con 2 generaciones gratuitas por día. Sin compromiso, sin tarjeta de crédito.',
    'whatClientsSay': 'Lo que dicen nuestros clientes',
    'whatClientsSayDesc': 'Ve lo que nuestros usuarios están diciendo sobre nuestra plataforma',
    'viewAllFeedbacks': 'Ver todos los comentarios',
    'readyToStart': '¿Listo para comenzar?',
    'readyToStartDesc': 'Elige un plan y obtén acceso inmediato a todos nuestros servicios premium',
    'createFreeAccount': 'Crear Cuenta Gratuita',
    'affiliates': 'Afiliados',
    'raffles': 'Sorteos',
    'support': 'Soporte',
    'feedbacks': 'Comentarios',
    'redeemKey': 'Canjear Clave',
    'settings': 'Configuración',
    'chat': 'Chat',
    'tickets': 'Tickets',
    'administrator': 'Administrador',
    'user': 'Usuario',
    'allRightsReserved': 'Todos los derechos reservados',
    'welcome': 'Bienvenido',
    'viewProfile': 'Ver Perfil',
    'expiresIn': 'Expira en',
    'durationLabel': 'Duración',
    'days': 'días',
    'generationsLabel': 'Generaciones',
    'unlimitedLabel': 'Ilimitadas',
    'freePlanLabel': 'Plan Gratuito',
    'youAreUsingFreePlan': 'Estás usando el plan gratuito',
    'dailyGenerations': 'Generaciones diarias',
    'free': 'gratis',
    'upgradeToPremium': 'Actualizar a Premium',
    'generating': 'Generando...',
    'youDontHaveActivePlan': 'No tienes un plan activo',
    'youHave2FreeGenerations': '💡 ¡Tienes 2 generaciones gratuitas por día!',
    'accountGeneratedSuccess': '¡Cuenta Generada con Éxito! ✅',
    'accountFormat': 'Cuenta (formato: cuenta:contraseña):',
    'copyFullAccount': 'Copiar cuenta completa',
    'emailUser': 'Email/Usuario:',
    'usernameLabel': 'Usuario:',
    'emailUserCopied': '¡Email/Usuario copiado!',
    'passwordLabel': 'Contraseña:',
    'passwordCopied': '¡Contraseña copiada!',
    'copyButton': '📋 Copiar',
    'importantInfo': 'ℹ️ Información Importante:',
    'accountNotWorkingInfo': 'Si la cuenta no funciona, ¡no hay problema! Puedes generar de nuevo. A veces el inventario puede estar expirando o alguien puede haber cambiado la contraseña.',
    'saveCredentials': '⚠️ Importante: Guarda estas credenciales en un lugar seguro. No se mostrarán nuevamente.',
    'yourAffiliateLink': 'Tu Enlace de Afiliado',
    'linkToShare': 'Enlace para compartir:',
    'copyLink': '📋 Copiar Enlace',
    'affiliateTip': '💡 Consejo: ¡Comparte este enlace con tus amigos! Cuando se registren a través de tu enlace, obtienes 2 generaciones gratuitas y ellos también obtienen 2 generaciones gratuitas!',
    'viewFullAffiliateStats': 'Ver estadísticas completas de afiliados →',
    'availableServices': 'Servicios Disponibles',
    'availableLabel': 'disponibles',
    'noServicesAvailable': 'No hay servicios disponibles en este momento.',
    'errorLoadingServices': 'Error al cargar servicios',
    'errorGeneratingAccount': 'Error al generar cuenta',
    'affiliateLinkCopied': '¡Enlace de afiliado copiado!',
    'adminPanel': 'Panel Administrativo',
    'update': 'Actualizar',
    'totalUsers': 'Total de Usuarios',
    'totalRevenue': 'Ingresos Totales',
    'confirmedPayments': 'Pagos Confirmados',
    'availableStocks': 'Inventario Disponible',
    'activeServices': 'Servicios Activos',
    'generatedAccounts': 'Cuentas Generadas',
    'availableKeys': 'Claves Disponibles',
    'quickActions': 'Acciones Rápidas',
    'recentUsers': 'Usuarios Recientes',
    'recentPayments': 'Pagos Recientes',
    'noEmail': 'Sin email',
    'noPlan': 'Sin plan',
    'noRecentUsers': 'No hay usuarios recientes',
    'noRecentPayments': 'No hay pagos recientes',
    'errorLoadingStats': 'Error al cargar estadísticas',
    'loading': 'Cargando...',
    'activatePlanAuto': 'Activar plan automáticamente',
    'activatingPlan': 'Activando plan...',
    'planActivationPending': 'El plan aún no ha sido activado automáticamente',
    'planActivatedManualSuccess': '¡Plan activado con éxito!',
    'errorActivatingPlan': 'Error al activar plan',
    'shareYourExperience': '¡Comparte tu experiencia con nosotros! Tus comentarios ayudan a mejorar nuestros servicios.',
    'sendFeedback': 'Enviar Comentario',
    'nameOrUsername': 'Nombre (o usa tu nombre de usuario)',
    'yourName': 'Tu nombre',
    'ratingOptional': 'Calificación (opcional)',
    'message': 'Mensaje *',
    'shareExperiencePlaceholder': 'Comparte tu experiencia, sugerencias o cumplidos...',
    'charactersCount': 'caracteres (mínimo 10)',
    'sending': 'Enviando...',
    'sendFeedbackButton': 'Enviar Comentario',
    'feedbackWillBeReviewed': '⚠️ Tu comentario será revisado antes de ser publicado',
    'approvedFeedbacks': 'Comentarios Aprobados',
    'noFeedbackYet': 'Aún no hay comentarios',
    'beFirstToShare': '¡Sé el primero en compartir tu experiencia!',
    'errorLoadingFeedbacks': 'Error al cargar comentarios',
    'pleaseFillAllFields': 'Por favor, completa todos los campos obligatorios',
    'messageMinLength': 'El mensaje debe tener al menos 10 caracteres',
    'feedbackSentSuccess': '¡Comentario enviado con éxito! Espera la aprobación del administrador.',
    'errorSendingFeedback': 'Error al enviar comentario',
    'enterYourAccount': 'Ingresa a tu cuenta para continuar',
    'enterUsername': 'Ingresa tu nombre de usuario',
    'enterPassword': 'Ingresa tu contraseña',
    'loggingIn': 'Iniciando sesión...',
    'invalidCredentials': 'Credenciales inválidas',
    'loginSuccess': '¡Inicio de sesión exitoso!',
    'errorLoggingIn': 'Error al iniciar sesión',
    'dontHaveAccount': '¿No tienes una cuenta?',
    'backToHome': '← Volver a la página de inicio',
    'joinUs': 'Únete a nosotros y comienza ahora',
    'youWillGet2FreeGenerations': '🎁 ¡Obtendrás 2 generaciones gratuitas cuando te registres a través de este enlace!',
    'emailOptional': '(Opcional)',
    'enterEmail': 'Ingresa tu email',
    'confirmPassword': 'Confirmar Contraseña',
    'enterConfirmPassword': 'Ingresa tu contraseña nuevamente',
    'passwordsDontMatch': 'Las contraseñas no coinciden',
    'passwordMinLength': 'La contraseña debe tener al menos 6 caracteres',
    'accountCreatedSuccess': '¡Cuenta creada con éxito!',
    'accountCreatedButLoginError': 'Cuenta creada, pero error durante el inicio de sesión automático. Por favor, inicia sesión manualmente.',
    'errorCreatingAccount': 'Error al crear cuenta',
    'checkMongoDB': 'Verifica si MongoDB es accesible y DATABASE_URL es correcta en .env',
    'configureMongoDB': 'Configura MongoDB en el archivo .env y ejecuta: npm run db:push',
    'errorLoadingTickets': 'Error al cargar tickets',
    'ticketCreatedSuccess': '¡Ticket creado con éxito!',
    'errorCreatingTicket': 'Error al crear ticket',
    'enterMessage': 'Ingresa un mensaje',
    'replySent': '¡Respuesta enviada!',
    'errorSendingReply': 'Error al enviar respuesta',
    'errorLoadingTicketDetails': 'Error al cargar detalles del ticket',
    'open': 'Abierto',
    'inProgress': 'En Progreso',
    'resolved': 'Resuelto',
    'closed': 'Cerrado',
    'low': 'Baja',
    'medium': 'Media',
    'high': 'Alta',
    'urgent': 'Urgente',
    'subject': 'Asunto',
    'priority': 'Prioridad',
    'createTicket': 'Crear Ticket',
    'newTicket': 'Nuevo Ticket',
    'myTickets': 'Mis Tickets',
    'noTickets': 'No se encontraron tickets',
    'createFirstTicket': 'Crea tu primer ticket de soporte',
    'reply': 'Responder',
    'sendReply': 'Enviar Respuesta',
    'replies': 'Respuestas',
    'noReplies': 'Aún no hay respuestas',
    'creating': 'Creando cuenta...',
    'alreadyHaveAccount': '¿Ya tienes una cuenta?',
    'selectLanguage': 'Selecciona tu idioma',
    'selectLanguageDescription': 'Elige el idioma que hablas para que podamos asistirte mejor en Telegram.',
    'yourLanguage': 'Tu idioma',
    'otherLanguage': 'Otro idioma',
    'payManualInternational': 'Pago Manual (Internacional)',
    'manualPaymentInternational': 'Pago Manual Internacional',
    'manualPaymentDescription': 'Para usuarios fuera de Brasil, ofrecemos pago manual vía Telegram. Aceptamos Wise y cualquier criptomoneda.',
    'acceptedPaymentMethods': 'Métodos de pago aceptados:',
    'wiseDescription': 'Transferencia bancaria internacional vía Wise',
    'anyCryptocurrency': 'Cualquier Criptomoneda',
    'cryptoDescription': 'Bitcoin, Ethereum, USDT y otras criptomonedas',
    'manualPaymentInstructions': 'Haz clic en el botón de abajo para abrir Telegram. Envía un mensaje informando el plan deseado y el método de pago que prefieres usar. Responderemos rápidamente con las instrucciones de pago.'
  }
}

// Função para traduzir texto dinâmico (como mensagens de feedback)
export function useDynamicTranslation() {
  const router = useRouter()
  const locale = (router?.locale || 'pt-BR') as keyof typeof translations
  const [translatedTexts, setTranslatedTexts] = useState<Record<string, string>>({})
  const translatingRef = useRef<Set<string>>(new Set())

  const translate = useCallback(async (text: string): Promise<string> => {
    if (!text || locale !== 'en') {
      return text
    }

    // Verificar cache
    if (translatedTexts[text]) {
      return translatedTexts[text]
    }

    // Se já está traduzindo, retornar original
    if (translatingRef.current.has(text)) {
      return text
    }

    // Iniciar tradução
    translatingRef.current.add(text)
    try {
      const translated = await translateText(text, 'pt', 'en')
      setTranslatedTexts(prev => ({ ...prev, [text]: translated }))
      translatingRef.current.delete(text)
      return translated
    } catch (error) {
      translatingRef.current.delete(text)
      return text
    }
  }, [locale, translatedTexts])

  return { translate }
}

// Função para traduzir nomes de planos automaticamente
export function translatePlanName(planName: string, locale: string = 'pt-BR'): string {
  if (locale === 'pt-BR') {
    return planName // Retornar original se for português
  }

  const planNameLower = planName.toLowerCase().trim()
  
  // Mapeamento de nomes de planos (ordem importa - mais específicos primeiro)
  const planTranslations: Record<string, Record<string, string>> = {
    'en': {
      'plano diário': 'Daily Plan',
      'plano diaria': 'Daily Plan',
      'diário': 'Daily',
      'diaria': 'Daily',
      'daily': 'Daily',
      'plano semanal': 'Weekly Plan',
      'semanal': 'Weekly',
      'weekly': 'Weekly',
      'plano mensal': 'Monthly Plan',
      'mensal': 'Monthly',
      'monthly': 'Monthly',
      'plano anual': 'Annual Plan',
      'anual': 'Annual',
      'yearly': 'Annual',
      'plano permanente': 'Lifetime Plan',
      'plano vitalício': 'Lifetime Plan',
      'permanente': 'Lifetime',
      'lifetime': 'Lifetime',
      'vitalício': 'Lifetime',
      'lifelong': 'Lifetime'
    },
    'es': {
      'plano diário': 'Plan Diario',
      'plano diaria': 'Plan Diario',
      'diário': 'Diario',
      'diaria': 'Diario',
      'daily': 'Diario',
      'plano semanal': 'Plan Semanal',
      'semanal': 'Semanal',
      'weekly': 'Semanal',
      'plano mensal': 'Plan Mensual',
      'mensal': 'Mensual',
      'monthly': 'Mensual',
      'plano anual': 'Plan Anual',
      'anual': 'Anual',
      'yearly': 'Anual',
      'plano permanente': 'Plan Permanente',
      'plano vitalício': 'Plan Permanente',
      'permanente': 'Permanente',
      'lifetime': 'Permanente',
      'vitalício': 'Permanente'
    },
    'fr': {
      'plano diário': 'Plan Quotidien',
      'plano diaria': 'Plan Quotidien',
      'diário': 'Quotidien',
      'diaria': 'Quotidien',
      'daily': 'Quotidien',
      'plano semanal': 'Plan Hebdomadaire',
      'semanal': 'Hebdomadaire',
      'weekly': 'Hebdomadaire',
      'plano mensal': 'Plan Mensuel',
      'mensal': 'Mensuel',
      'monthly': 'Mensuel',
      'plano anual': 'Plan Annuel',
      'anual': 'Annuel',
      'yearly': 'Annuel',
      'plano permanente': 'Plan À vie',
      'plano vitalício': 'Plan À vie',
      'permanente': 'À vie',
      'lifetime': 'À vie',
      'vitalício': 'À vie'
    }
  }

  const translations = planTranslations[locale] || planTranslations['en'] || {}
  
  // Tentar encontrar correspondência exata primeiro (mais específica)
  for (const [key, translation] of Object.entries(translations)) {
    if (planNameLower === key || planNameLower.startsWith(key + ' ') || planNameLower.endsWith(' ' + key) || planNameLower.includes(' ' + key + ' ')) {
      // Substituir a palavra-chave pela tradução, mantendo o resto do nome
      const regex = new RegExp(key.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'gi')
      return planName.replace(regex, translation)
    }
  }

  // Se não encontrou correspondência exata, tentar substituição simples
  for (const [key, translation] of Object.entries(translations)) {
    if (planNameLower.includes(key)) {
      const regex = new RegExp(key.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'gi')
      return planName.replace(regex, translation)
    }
  }

  // Se não encontrou tradução, retornar original
  return planName
}

export function useTranslation() {
  const router = useRouter()
  const locale = (router?.locale || 'pt-BR') as keyof typeof translations
  const [translatedKeys, setTranslatedKeys] = useState<Record<string, string>>({})
  const [translatingKeys, setTranslatingKeys] = useState<Set<string>>(new Set())
  const translatingRef = useRef<Set<string>>(new Set())

  // Limpar cache quando o idioma mudar
  useEffect(() => {
    if (locale !== 'en' && locale !== 'es') {
      setTranslatedKeys({})
      translatingRef.current.clear()
      setTranslatingKeys(new Set())
    }
  }, [locale])

  const t = useCallback((key: string) => {
    // Se não for inglês ou espanhol, usar tradução estática
    if (locale !== 'en' && locale !== 'es') {
    return translations[locale]?.[key] || key
  }

    // Se for inglês, verificar se já tem tradução dinâmica
    if (translatedKeys[key]) {
      return translatedKeys[key]
    }

    // Verificar se já tem tradução estática em inglês
    const enTranslation = translations['en']?.[key]
    if (enTranslation && enTranslation !== key) {
      return enTranslation
    }

    // Se não tem tradução e não está traduzindo, iniciar tradução
    if (!translatingRef.current.has(key)) {
      const ptText = translations['pt-BR']?.[key]
      if (ptText && ptText !== key) {
        translatingRef.current.add(key)
        setTranslatingKeys(prev => new Set(prev).add(key))
        
        // Traduzir de forma assíncrona
        translateText(ptText, 'pt', 'en')
          .then(translated => {
            setTranslatedKeys(prev => ({ ...prev, [key]: translated }))
            translatingRef.current.delete(key)
            setTranslatingKeys(prev => {
              const newSet = new Set(prev)
              newSet.delete(key)
              return newSet
            })
          })
          .catch(() => {
            // Em caso de erro, usar tradução estática ou texto original
            setTranslatedKeys(prev => ({ ...prev, [key]: enTranslation || ptText }))
            translatingRef.current.delete(key)
            setTranslatingKeys(prev => {
              const newSet = new Set(prev)
              newSet.delete(key)
              return newSet
            })
          })
      }
    }

    // Retornar tradução estática enquanto traduz ou texto original
    return enTranslation || translations['pt-BR']?.[key] || key
  }, [locale, translatedKeys])

  return {
    t,
    locale,
    translating: translatingKeys.size > 0,
    changeLanguage: (newLocale: string) => {
      router.push(router.asPath, router.asPath, { locale: newLocale })
    },
    translatePlanName: (planName: string) => translatePlanName(planName, locale)
  }
}
